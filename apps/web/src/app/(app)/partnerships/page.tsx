...
export default function CategoryPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-[var(--text-1)] mb-1">Strategic Partnerships & Co-Bidding</h1>
        <p className="text-sm text-[var(--text-2)]">Consortium co-bidding opportunities, prime contractor partnerships, and alliances.</p>
      </div>

      <div className="max-w-md">
        <Input
          placeholder="Search strategic partnerships & co-bidding..."
          value={search}
      ...
      {isError ? (
        ...
        <EmptyState preset="no-opportunities" description="No strategic partnerships & co-bidding match your current search." action={{ label: 'Clear Search', onClick: () => setSearch('') }} />
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {data.data.map((opp) => (
